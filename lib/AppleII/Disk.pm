...
# Created: 25 Jul 1996
# Version: $Revision: 0.1 $ ($Date: 1996/07/26 02:35:51 $)
#
...
#   pad:    The value to pad the block with (optional)
...
sub readBlock
{
    ...
    my $pos = $block * 0x200;
#    return "\0" x 0x200 if $pos > $self->{actlen}; # Past EOF
    croak("Invalid block number $block") if $pos < 0 or $pos > $self->{actlen};

    $file->seek($pos,0) or die;
    my $buffer = '';
    read($file,$buffer,0x200) or die;
...
sub readBlock
{
    ...
    print STDERR "Reading block $block\n" if $debug;
...
#    return "\0" x 0x100 if $pos > $self->{actlen}; # Past EOF
    croak("Invalid position track $track sector $sector")
        if $pos < 0 or $pos > $self->{actlen};

    print STDERR "Reading track $track sector $sector\n" if $debug;

    $file->seek($pos,0) or die;
    my $buffer = '';
    read($file,$buffer,0x100) or die;
